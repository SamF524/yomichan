...
class AnkiTemplateRenderer {
  ...
  async prepare() {
    this._templateRenderer.registerHelpers([
      ...
      ['wanikani', this._wanikani.bind(this)],
    ])
    ...
    await Promise.all([this._structuredContentStyleApplier.prepare(), this._pronunciationStyleApplier.prepare()])
  }
  ...
  _sanitizeCssClass(context, options) {
    return options.fn(context).replace(/[^_a-z0-9\u00a0-\uffff]/gi, '_')
  }
  ...
  _op(context, ...args) {
    switch (args.length) {
      case 3:
        return this._evaluateUnaryExpression(args[0], args[1])
      case 4:
        return this._evaluateBinaryExpression(args[0], args[1], args[2])
      case 5:
        return this._evaluateTernaryExpression(args[0], args[1], args[2], args[3])
      default:
        return void 0
    }
  }
  ...
  _normalizeHtml(root, styleApplier, datasetKeyIgnorePattern) {
    ...
    for (const element of elements) {
      ...
      for (const key of Object.keys(dataset)) {
        if (datasetKeyIgnorePattern !== null && datasetKeyIgnorePattern.test(key)) {
          continue
        }
        delete dataset[key]
      }
  ...
  _formatGlossary(context, dictionary, options) {
    ...
    if (typeof content === 'string') {
      return this._stringToMultiLineHtml(this._escape(content))
    }
    ...
    switch (content.type) {
      case 'image':
        return this._formatGlossaryImage(content, dictionary, data)
      case 'structured-content':
        return this._formatStructuredContent(content, dictionary, data)
    }
    return ''
  }
  ...
  _pronunciation(context, ...args) {
    ...
    if (typeof downstepPosition !== 'number') {
      return ''
    }
    if (!Array.isArray(nasalPositions)) {
      nasalPositions = []
    }
    if (!Array.isArray(devoicePositions)) {
      devoicePositions = []
    }
    ...
    switch (format) {
      case 'text':
        return this._getPronunciationHtml(
          this._pronunciationGenerator.createPronunciationText(
            morae,
            downstepPosition,
            nasalPositions,
            devoicePositions,
