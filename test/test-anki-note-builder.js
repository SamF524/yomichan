...
async function createVM() {
  ...
  const [JapaneseUtil, AnkiNoteBuilder, AnkiTemplateRenderer] = vm.get([
    'JapaneseUtil',
    'AnkiNoteBuilder',
...
async function getRenderResults(dictionaryEntries, type, mode, template, AnkiNoteBuilder, JapaneseUtil, write) {
  ...
  for (const dictionaryEntry of dictionaryEntries) {
    ...
    const {
      ...
      errors,
    } = await ankiNoteBuilder.createNote({
      dictionaryEntry,
      mode: null,
      context,
      template,
...
async function main() {
  ...
  const template = fs.readFileSync(
    path.join(__dirname, '..', 'ext', 'data/templates/default-anki-field-templates.handlebars'),
    { encoding: 'utf8' },
  )
      ...
      case 'findKanji':
        {
          ...
          const results = clone(
            await getRenderResults(dictionaryEntries, 'kanji', null, template, AnkiNoteBuilder, JapaneseUtil, write),
          )
          actualResults1.push({ name, results })
          if (!write) {
            assert.deepStrictEqual(results, expected1.results)
